...
static bool check_win(const struct forest *frst) {
  ...
  for (int i = 0; i < frst->width * frst->height; i++) {
    if (frst->ghost_map[i] == ghost_absence && frst->player_map[i] == UNREVEALED) {
      return false; 
    }

    if (frst->ghost_map[i] == ghost_presence && (frst->player_map[i] != MARKED && frst->player_map[i] != UNREVEALED)) {
      return false;
    }
  }
  return true;
...
int forest_reveal(struct forest *frst, int x, int y) {
  ...
  if(frst->ghost_map[pos]==ghost_presence) {
    frst->player_map[pos] = 'G';
